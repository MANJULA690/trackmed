...
import {
  ...
  formatCurrency, CATEGORIES, TRANSACTION_TYPES,
} from "../utils/helpers";
...
/* ── Portal modal — never scrolls with the page ─────────── */
function Modal({ open, onClose, title, children, footer, maxW = 640 }) {
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      ...
      <div style={{
        position: "relative", background: "#fff", borderRadius: 20, width: "100%", maxWidth: maxW,
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 22px", borderBottom: "1px solid #f1f5f9", flexShrink: 0, background: "#fff" }}>
          ...
          <button onClick={onClose} style={{ border: "none", background: "#f1f5f9", color: "#64748b", padding: 6, borderRadius: 8, cursor: "pointer", display: "flex" }}>
        ...
        {/* Sticky footer */}
        {footer && (
          <div style={{ flexShrink: 0, background: "#fff", borderTop: "1px solid #f1f5f9", padding: "14px 22px" }}>
            {footer}
          </div>
        )}
      </div>
...
function MedicineNameSearch({ value, onChange, onSelect }) {
  ...
  const handleType = (val) => {
    setQuery(val);
    onChange(val);
    if (!val.trim()) { setSuggestions([]); setOpen(false); }
  };
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative" }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        />
        {fetching && (
          <svg style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", animation: "spin 0.8s linear infinite", color: "#00B5AD" }}
...
export default function Inventory() {
  ...
  const handleStock = async (e) => {
    ...
    try {
      ...
      toast.success("Stock updated!"); setStockMed(null);
  ...
  const handleAutofill = (med) => {
    setForm(f => ({
      ...
      genericName:      med.genericName      || med.composition1 || f.genericName,
  ...
  const sectionTitle = (t) => (
    <div style={{ fontSize: 10, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12, marginTop: 4, display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ flex: 1, height: 1, background: "#f1f5f9" }}/>{t}<span style={{ flex: 1, height: 1, background: "#f1f5f9" }}/>
    </div>
  );
  ...
  return (
    <div>
                ...
                {medicines.map((med) => {
                  ...
                  return (
                    <tr key={med._id} style={{ cursor: "pointer" }} onClick={() => setDetailMed(med)}>
                      <td>
                        <p style={{ fontWeight: 600, color: "#0f172a", fontSize: 13 }}>{med.name}</p>
                        {med.genericName && <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{med.genericName}</p>}
                      </td>
                      ...
                      <td>
                        <span style={{ fontWeight: 700, color: "#0f172a" }}>{med.quantity}</span>
                        <span style={{ fontSize: 11, color: "#94a3b8", marginLeft: 3 }}>{med.unit}</span>
                      </td>
                      ...
                      <td onClick={e => e.stopPropagation()}>
                        <div style={{ display: "flex", gap: 4 }}>
                          {[
                            { title:"Update stock", color:"#00B5AD", bg:"#e0f7f6", fn:() => setStockMed(med),
                              svg:<><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></> },
                            { title:"Edit", color:"#6366f1", bg:"#eef2ff", fn:() => openEdit(med),
                            ...
                            { title:"Delete", color:"#ef4444", bg:"#fee2e2", fn:() => setDeleteTarget(med),
                              svg:<><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></> },
                          ].map(({ title, color, bg, fn, svg }) => (
                            <button key={title} onClick={fn} title={title}
                              ...
                              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">{svg}</svg>
                            </button>
                          ))}
                        </div>
                      </td>
      ...
      <Modal open={addOpen || !!editMed} onClose={() => { setAddOpen(false); setEditMed(null); }}
        ...
        footer={
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button type="submit" form="med-form" disabled={submitting} className="btn-primary" style={{ flex: 1 }}>
              {submitting ? "Saving..." : editMed ? "Save Changes" : "Add Medicine"}
            </button>
          </div>
        }>
        <form id="med-form" onSubmit={editMed ? handleEdit : handleAdd} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {sectionTitle("Medicine Identity")}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ gridColumn: "span 2" }}>
              <label style={L}>
                Medicine name *
                {!editMed && <span style={{ color: "#00B5AD", fontWeight: 400, fontSize: 11, marginLeft: 6 }}>— type to search from database</span>}
              </label>
              {editMed
                ? <input required value={form.name} onChange={e => F("name", e.target.value)} className="input-base" placeholder="e.g. Paracetamol 500mg" />
                : <MedicineNameSearch value={form.name} onChange={v => F("name", v)} onSelect={handleAutofill} />
              }
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><label style={L}>Opening quantity *</label><input required type="number" min="0" value={form.quantity} onChange={e => F("quantity", e.target.value)} className="input-base" placeholder="0" /></div>
            ...
            <div><label style={L}>Price (₹)</label><input type="number" min="0" step="0.01" value={form.price} onChange={e => F("price", e.target.value)} className="input-base" placeholder="0.00" /></div>
      ...
      {/* ── Stock update Modal ───────────────────────────── */}
      <Modal open={!!stockMed} onClose={() => setStockMed(null)}
        title={`Update Stock — ${stockMed?.name}`} maxW={400}
        footer={
          <div style={{ display: "flex", gap: 10 }}>
