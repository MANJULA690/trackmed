...
/* ── FIX 2: correct alertType values for each filter ─────── */
const FILTER_CONFIG = {
  all:      { label: "All",      isResolved: false, alertTypes: null },
  critical: { label: "Critical", isResolved: false, alertTypes: ["expiry_critical", "out_of_stock", "expired"] },
  warning:  { label: "Warning",  isResolved: false, alertTypes: ["expiry_warning", "low_stock"] },
  resolved: { label: "Resolved", isResolved: true,  alertTypes: null },
};
...
export default function Alerts() {
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
      ...
      /* Build query params */
      const params = {
        isResolved: String(cfg.isResolved),
        page,
        limit: 20,
      };

      /* FIX 2: Pass severity instead of alertType — backend supports severity filter */
  ...
  useEffect(() => {
    fetchAlerts();
    /* Auto-refresh every 30s to stay in sync with inventory */
    const id = setInterval(fetchAlerts, 30000);
    return () => clearInterval(id);
  }, [fetchAlerts]);
  ...
  const handleScan    = async ()   => {
    const tid = toast.loading("Scanning inventory...");
    try { await alertAPI.triggerScan(); toast.dismiss(tid); toast.success("Scan complete!"); fetchAlerts(); refetchAlerts?.(); }
    catch { toast.dismiss(tid); toast.error("Scan failed"); }
  };
  ...
  return (
    <div>
      ...
      {loading ? <PageLoader /> : alerts.length === 0 ? (
        <div className="card" style={{ padding: 0 }}>
          <EmptyState
            icon={filter === "resolved" ? "✅" : "🎉"}
            title={filter === "resolved" ? "No resolved alerts" : `No ${filter === "all" ? "" : filter + " "}alerts`}
            subtitle={filter === "resolved" ? "Nothing resolved yet" : "All medicines are within safe limits"}
          />
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {alerts.map((alert, idx) => {
            ...
            return (
              <div key={alert._id}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 13.5, fontWeight: 700, color: alert.isResolved ? "#6b7280" : "#1e293b" }}>
                      {alert.medicineName}
                    </span>
                    <span className={alert.severity === "critical" ? "pill-critical" : "pill-low"}>
                      {alert.alertType.replace(/_/g, " ")}
                    ...
                    {alert.isResolved && <span style={{ fontSize: 11, color: "#10b981", fontWeight: 600 }}>✓ Resolved</span>}
                  </div>
                  <p style={{ fontSize: 12.5, color: "#64748b", marginBottom: 4, lineHeight: 1.5 }}>{alert.message}</p>
                  <p style={{ fontSize: 11, color: "#94a3b8" }}>{timeAgo(alert.createdAt)}</p>
                </div>
